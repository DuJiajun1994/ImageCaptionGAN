...
class Discriminator(nn.Module):
    def __init__(self, args):
        ...
        self.bilstm = nn.LSTM(self.encoding_size,self.rnn_size*2, self.num_layers,dropout = 0.2, bidirectional=True, bias=False)
        ...
        self.out_fc = nn.Sequential(nn.Linear(3*args.rnn_size,2),nn.Dropout(self.drop_prob))
    ...
    def forward(self, fc_feats, att_feats, att_masks, seqs):
        ...
        output = self.bilstm(batch_sequence) # bsz * seq_length * self.rnn_size*2
        ...
        M = torch.bmm(att_w,output)  # bsz * self.att_num *  self.rnn_size*2
        output = self.relu(self.fc(M.view(bsz, -1)))
        ...
        probs = F.softmax(self.out_fc(output))
